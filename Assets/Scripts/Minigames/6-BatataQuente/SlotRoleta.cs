...
public class SlotRoleta : MonoBehaviour
{
    [Serializable]
    ...
    public void SetEntriesFromSteamIds(IEnumerable<ulong> steamIds)
    {
        ...
        {
            var pd = PlayerList.singleton.players.FirstOrDefault(p => p.playerInfo.steamId == sid);
            if (pd == null) continue;

            entradas.Add(new Entrada {
                playerData = pd,
                sprite = pd.icon,
                peso = 1f // (se quiser pesos visuais, ajuste aqui)
            });
        }
    ...
    public void SpinToWinner(ulong steamId)
    {
        ...
        int vencedorCatalog = entradas.FindIndex(e => e.playerData && e.playerData.playerInfo.steamId == steamId);
            ...
            .setOnComplete(() =>
            {
                LeanTween.value(content.gameObject, content.anchoredPosition.x, targetX, tBack)
                    ...
                    .setOnComplete(() =>
                    {
                        ...
                        ShowWinText(vencedor.playerData.alias);
                    });
            });
    }
    ...
    void ShowWinText(string rotulo)
    {
        ...
        seq.append( () => winText.gameObject.SetActive(false) );
    }
